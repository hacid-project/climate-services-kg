import re
import string
from pyrml import PyRML, rml_function, CSVSource, FunctionMap, PredicateObjectMap, TripleMappings
import pyrml.rml_vocab as rml_vocab
import pyrml.pyrml_core as pyrml_core
import pyrml.pyrml_api as pyrml_api
import itertools
import numpy as np
import json
...
# Restore the behaviour of previous pyrml versions, where a term map (e.g. a
# subject map based on a function) could produce a list of terms, generating
# one triple per term. The current version only expands lists of objects.
_triple_mapping_apply = TripleMappings.apply

def _as_list(term):
    if isinstance(term, (list, np.ndarray)):
        return [t for sub_term in term for t in _as_list(sub_term)]
    return [] if term is None or pd.isna(term) else [term]

def _triple_mapping_apply_expanding_lists(self, *args, **kwargs):
    return [
        expanded_tuple
        for _tuple in _triple_mapping_apply(self, *args, **kwargs)
        for expanded_tuple in itertools.product(*map(_as_list, _tuple))
    ]

TripleMappings.apply = _triple_mapping_apply_expanding_lists

# Keep the IRIs generated by previous pyrml versions: values inserted in IRI
# templates are percent-encoded leaving '/' unescaped (e.g. .../W/m2), while
# the current version also escapes '/' (e.g. .../W%2Fm2).
_quote = pyrml_api.quote
pyrml_api.quote = lambda string, safe='/': _quote(string, safe='/')

# Workaround for a pyrml-lib inconsistency: boolean functions (e.g.
# idlab:notEqual) return Python booleans, which become "True"/"False"
# literals when nested, while grel:controls_if only accepts "true".
_CONTROLS_IF = 'http://users.ugent.be/~bjdmeest/function/grel.ttl#controls_if'
PyRML.unregister_function(_CONTROLS_IF)

@rml_function(fun_id=_CONTROLS_IF,
...
def controls_if(cond, e_true, e_false=None):
